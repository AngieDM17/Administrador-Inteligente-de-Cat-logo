...
def redactar_guion_voz(ficha: dict) -> str | None:
    ...
    prompt = (
        "Redacta el CUERPO de un guion de voz en off PUBLICITARIO Y AGRESIVO "
        "A LA VENTA, en espanol colombiano neutro, para un video de producto "
        "de la tienda Ekipon. El objetivo NO es informar como una ficha "
        "tecnica leida en voz alta: el objetivo es que quien escuche piense "
        "'la quiero, la necesito' y compre. Presupuesto estricto: como "
        f"maximo {PRESUPUESTO_CUERPO_GUION} caracteres (contando espacios). "
        "Devuelve SOLO el texto del guion, sin comillas, sin titulo, sin "
        "explicaciones.\n\n"
        "COMO VENDER (sin inventar nada):\n"
        "- Empieza con un gancho de deseo o de problema resuelto, NUNCA con "
        "un dato tecnico suelto (nada de arrancar con 'Cuenta con...' o "
        "'Tiene...').\n"
        "- Traduce cada caracteristica en el BENEFICIO real para quien "
        "compra (que gana, que se ahorra, que problema deja de tener) en "
        "vez de solo nombrar el dato.\n"
        "- Tono directo, urgente, hablado -- como alguien que esta "
        "convenciendo en persona, no un catalogo. Frases cortas.\n\n"
        ...
        "los datos de abajo. La venta se construye en COMO SE CUENTA lo "
        "que ya es real, no agregando datos nuevos. Si los datos no "
        "alcanzan para llenar el presupuesto, escribe un guion mas corto: "
        "nunca rellenes con datos inventados ni genericos.\n\n"

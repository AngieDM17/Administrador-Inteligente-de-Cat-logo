...
**Etapa de IMÁGENES: CERRADA end-to-end (23-jul, borrador 50255 con galería de 8).** Quedan mejoras, no el cierre: automatizar el handoff del sourcing, confirmar dimensiones reales. **30-jul:** los generadores `persona_escala`/`escena_funcionamiento`/`otro_angulo_ia`/`portada_variantes` quedaron **fuera de alcance por decisión de Angie** — se generan aparte y se suben a mano; se retoman cuando ese plan esté mejor montado. **Regla dura:** todo sourcing automático pasa por un gate de verificación visual (bajar fotos por URL a ciegas trajo OTROS productos). Detalle en `ETAPA_IMAGENES.md`.

**Fase de ESCALA: EMPEZADA (27-28 jul).** Se midió la tasa de error del Investigador en dos lotes de links de Alibaba (10 agro → 50% necesita corrección; 10 mixtos → 30% limpio / 70% corrección). Hallazgo clave: el pipeline NO se rompe; el cuello de botella es la **calidad de la fuente** (Alibaba: campos autollenados que contradicen el nombre, listings configurables/familia, specs delgadas), no el Investigador. Conclusión medida: la revisión humana es obligatoria. Sobre esa base se construyó el **colador de listo-para-publicar** (`revisor_publicacion.py`, commit 1d171c6): lee las notas que el Investigador ya deja en la ficha (`advertencias`, `campos_por_confirmar`) + chequeos mecánicos (potencia, dimensiones, estado usado) y marca cada ficha **LISTO** o **REVISAR**. Hoy marca el 100% de las fichas existentes (todas tienen ≥1 cosa real por confirmar): no recorta el número a revisar todavía, cambia el CÓMO (cada ficha llega pre-etiquetada); el verde aparece cuando entren fichas más limpias.

**Próximo paso (Prioridad A restante):** (1) cerrar el círculo end-to-end del colador (un link → Investigador arma ficha → colador la revisa, de una sola pasada); (2) decidir si "sin dimensiones" bloquea o solo avisa; (3) producción tienda real = **decisión de encender** (ampliar el candado a `ekipon.co` en código + credenciales reales, tarea de Angie — Claude no toca credenciales). Seguridad ya cumplida en parte: `.env` fuera del repo (verificado 22-jul) y candado `TIENDAS_PERMITIDAS = {"pruebas.ekipon.co"}` vigente en `cliente_tienda.py` (verificado 28-jul); falta rotar las claves reales.
